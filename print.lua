...
local Window = Rayfield:CreateWindow({
	...
	ConfigurationSaving = {
        ...
        Discord = {
        	Enabled = false,
        	Invite = "KPKnVsG5km", -- The Discord invite code, do not include discord.gg/
	...
	KeySettings = {
		Title = "Cat Hub",
		Subtitle = "Key System",
		Note = "Key in this discord server (discord.gg/KPKnVsG5km)",
		FileName = "",
		SaveKey = true,
...
function GetCoinsH1()
    ...
    local ListCoins = Invoke("Get Coins")
    for i,v in pairs(ListCoins) do
       if  string.find(v.n, "Bonus") then if (v.p - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 300 then
            local shit = v
            shit["index"] = i 
            table.insert(returntable, shit)
         end  end
...
if #serverList > 0 then
    ...
    function GetMyEquipedPets()
        local equipedPets = {}
        for i,v in pairs(EquippedPets()) do
            table.insert(equipedPets,v.uid)
        end
        return equipedPets
    end

local function TP_Mode(arg)
        for i,v in pairs(game.workspace["__THINGS"].Coins:GetChildren()) do
            if tostring(arg) == v.Name then
                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.Coin.CFrame + Vector3.new(0,10,0)
            end
        end
        wait(0.5)
    end 
        local function GetCoinsComet()
        local returntable = {}
        local ListCoins = Invoke("Get Coins")
        for i,v in pairs(ListCoins) do
                    if string.find(tostring(v.n), "Comet") then
                    local shit = v
                    shit["index"] = i
                    table.insert(returntable, shit)
                    AllCoins = i
...
Tab7:AddLabel("Username: "..game.Players.LocalPlayer.name)

Tab7:AddLabel("User Id: "..game.Players.LocalPlayer.UserId)

Tab7:AddLabel("Game Name: "..game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name)
...
Tab1:AddDropdown({
    Name = "Type",
    Default = "",
    Options = {"Normal","Chest","Multi Target","Highest Value","Lowest Value","Nearest","Gems","Easter Coins"},
    Callback = function(r)
...
Tab1:AddButton({
	Name = "Teleport To Event Island",
	Callback = function()
      		game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(44.6098251, 850.355103, 174.728439, -0.555343509, 6.28683061e-09, 0.831621051, 1.56618984e-08, 1, 2.89904101e-09, -0.831621051, 1.46347281e-08, -0.555343509)
  	end    
})
...
elseif FarmingType == "Easter Coins" then
...
local GTime = Tab1:AddLabel("Gift Time:")
spawn(function()
    while task.wait() do
...
for i=1,12 do
a = a+1 
Invoke("Redeem Free Gift",a) wait(.5) end a = 0 end end
})
...
local Section = Tab8:AddSection({
	Name = "World/Server Hop:"
})

Tab8:AddToggle({
    Name = "Auto Farm Comets",
    Default = false,
    Callback = function(hitler)
        getgenv().adolf = hitler
        while adolf and task.wait() do
            GetCoinsComet()
                local coin = GetCoinsComet()[1]
                if coin ~= nil then
                for i=1,10 do 
                    TP_Mode(coin.index)
                    wait(.2) end
                getgenv().nocomets = false
                getgenv().canhop = false
                Invoke("Join Coin",coin["index"],GetMyEquipedPets())

                for i,v in pairs(GetMyEquipedPets()) do
                    Fire("Farm Coin",coin["index"],v)
                end
                repeat task.wait() until not game:GetService("Workspace")["__THINGS"].Coins:FindFirstChild(coin["index"]) or adolf == false
else getgenv().nocomets = true end end end
})
...
Tab8:AddToggle({
    ...
    Default = false,
    Callback = function(hitler)
getgenv().Spawn = hitler
end})
Tab8:AddToggle({
    Name = "Fantasy World",
    Default = false,
    Callback = function(hitler)
getgenv().Fantasy = hitler
end})
Tab8:AddToggle({
    Name = "Tech World",
    Default = false,
...
Tab8:AddToggle({
    ...
    Default = false,
    Callback = function(hitler)
getgenv().Axolotl = hitler
end})
Tab8:AddToggle({
    Name = "Pixel World",
    Default = false,
    Callback = function(hitler)
getgenv().Pixel = hitler
end})
Tab8:AddToggle({
    Name = "Cat World",
    Default = false,
...
Tab8:AddToggle({
    ...
    Default = false,
    Callback = function(hitler)
getgenv().Doodle = hitler
end})
Tab8:AddToggle({
    Name = "Kawaii World",
    Default = false,
    Callback = function(hitler)
getgenv().Kawaii = hitler
end})
Tab8:AddToggle({
    Name = "World Hop",
    Default = false,
    Callback = function(hitler2)
getgenv().worldhop = hitler2
while worldhop and task.wait() do 
if syn then 
syn_context_set(2)
else
setthreadcontext(2)
end
if getgenv().Spawn and getgenv().nocomets then
Library.WorldCmds.Load("Spawn")
wait(5)
end
if getgenv().Fantasy and getgenv().nocomets then 
Library.WorldCmds.Load("Fantasy")
wait(5)
end
if getgenv().Tech and getgenv().nocomets then 
Library.WorldCmds.Load("Tech")
wait(5)
end
if getgenv().Axolotl and getgenv().nocomets then 
Library.WorldCmds.Load("Axolotl Ocean")
wait(5)
end
if getgenv().Pixel and getgenv().nocomets then 
Library.WorldCmds.Load("Pixel")
wait(5)
end
if getgenv().Cat and getgenv().nocomets then 
Library.WorldCmds.Load("Cat")
wait(5)
end
if getgenv().Doodle and getgenv().nocomets then 
Library.WorldCmds.Load("Doodle")
wait(5)
end
if getgenv().Kawaii and getgenv().nocomets then 
Library.WorldCmds.Load("Kawaii")
wait(5)
end
if getgenv().serverhop2 and getgenv().nocomets then
    getgenv().canhop = true
    else getgenv().canhop = false
end
end
end})
Tab8:AddToggle({
    Name = "Server Hop",
    Default = false,
    Callback = function(niger222)
        getgenv().serverhop2 = niger222
        while serverhop2 and wait() do 
if getgenv().canhop then
for i=1,50 do 
    serverhop() wait(.1) end
end end
end 
})
...
local EggInfoSection = Tab:AddSection({
	Name = "Egg Information:"
})

local EggTracker = Tab:AddLabel("Egg Info: don't select")
local EggOpened = Tab:AddLabel("Eggs Opened: don't select")
local EggAvaiable = Tab:AddLabel("Eggs Can Open: don't select")
...
Tab8:AddButton({
    Name = "Get Visual All Gamepasses",
    Callback = function(v)
        GetGamepasses()
    end
})
...
Tab6:AddLabel("Library | Orion")

Tab6:AddLabel("Creator Information:")

Tab6:AddDropdown({
	Name = "Creators",
	Default = "",
	Options = {"senpaigm#3881"},
	Callback = function()
